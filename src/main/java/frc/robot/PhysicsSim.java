...
public class PhysicsSim {
    ...
    private static class TalonSRXSimProfile extends SimProfile {
        ...
        public void run() {
            ...
            // _talon.getSimCollection().setSupplyCurrent(123.45);

            //_talon.getSimCollection().setSupplyCurrent(Math.abs(outPerc) * 30 * random(0.95, 1.05));
            double voltage = 12 - outPerc * outPerc * 3 / 4 * random(0.95, 1.05) ;
//            _talon.getSimCollection().setBusVoltage(voltage);
//            System.out.format("   Physics sim computed voltage is %4.2f%n", voltage) ;
        }
    }
